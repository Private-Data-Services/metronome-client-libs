  ...
  class ExpiryResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Metronome::ExpiryResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Metronome::ExpiryResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'complete')
        if (value = attributes[:'complete']).is_a?(Array)
          self.complete = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @expiry_date.to_s.length > 8
        invalid_properties.push('invalid value for "expiry_date", the character length must be smaller than or equal to 8.')
      end

      if @expiry_date.to_s.length < 8
        invalid_properties.push('invalid value for "expiry_date", the character length must be great than or equal to 8.')
      end
    ...
    def valid?
      ...
      return false if @expiry_date.to_s.length > 8
      return false if @expiry_date.to_s.length < 8
      return false if @expiry_date !~ Regexp.new(/^\d{4}(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])$/)
    ...
    def hash
      [expiry_date, pending, complete].hash
    end
