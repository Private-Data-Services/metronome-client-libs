...
export type AvailableRegion = "eu-north-1" | "coming-soon";
...
export class ServerConfiguration<T extends { [key: string]: string }> implements BaseServerConfiguration {
    ...
    public constructor(private customerID: string, private region: AvailableRegion, private variableConfiguration: T, useProdInstance: boolean = true) {
        if (/[a-zA-Z0-9\-]+/.test(customerID)) {
            this.url = "https://" + customerID + (useProdInstance ? ".api." :".dev-api.") + region + ".privatedataservices.com";
        } else {
            throw new Error("Invalid characters in customer ID.  Only [a-z0-9\-] allowed.");
        }
    }
